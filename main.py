...
def load_config():
    ...
    merged["note_mark_re"] = re.compile(
        f"({"|".join(map(re.escape, merged["note_marks"]))})"
    )
    return merged
...
def init_dropbox(config):
    ...
    try:
        ...
        print(
            f"Couldn't auth to Dropbox. :( Here's what I know:\n  {e}\nDouble-check your token?"
        )
        sys.exit(1)
    else:
...
def get_since_date(config):
    ...
    while True:
        ...
        try:
            ...
            print(
                "Didn't understand that. Try YYYY-MM-DD or hit enter to accept the default."
            )
...
def adjust_note(note):
    while True:
        ...
        command = (
            input(
                f"\n({note.date}) Add context (b)efore or (a)fter, make (n)ote, (s)kip> "
            )
            or "?"
        )
        match command[0]:
